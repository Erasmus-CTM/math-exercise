(function () {
  ...
  async function ensureSympy() {
    ...
    await mainPyodide.runPythonAsync([
      ...
      '    missing = max(0, target_dimension - rank)',
      '    plural = singular + "s"',
      '    if score >= 1.0 - 1e-9:',
      '        feedback = ""',
      '    elif count == 0:',
      ...
      '                    f"Remove or replace {plural}" + (f" and add {missing} missing independent direction(s)." if missing else "."))',
      '    elif invalid_count:',
      '        feedback = f"Remove or replace the red {plural}; each is zero or outside the {space_name}."',
      '        if missing:',
      '            feedback += f" Then add {missing} missing independent direction(s)."',
      '    elif dependent and missing:',
      '        feedback = (f"Every yellow {singular} belongs to the {space_name}, but the family is linearly dependent "',
      '                    f"and spans only {rank} of {target_dimension} required directions. "',
      '                    f"Choose an independent subset, then add {missing} missing direction(s).")',
      '    elif dependent:',
      '        feedback = (f"Every yellow {singular} belongs to the {space_name}, but the family is linearly dependent. "',
      ...
      '                    f"Add {missing} more independent {singular}" + ("s." if missing != 1 else "."))',
      '',
      '    return {"score": score, "feedback": feedback, "assessment": {name: {axis: statuses}}}',
      '',
      'def _math_check():',
      '    try:',
      '        mode = _math_mode',
