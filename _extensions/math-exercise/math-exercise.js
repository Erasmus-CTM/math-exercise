(function () {
  ...
  // ---------------------------------------------------------------------------
  // Explicit learning context for AI feedback
  // ---------------------------------------------------------------------------
  ...
  function collectFeedbackContexts(cell) {
    var raw = cell.dataset.feedbackContext || '';
    if (!raw.trim()) return [];
    ...
    raw.split(',').forEach(function (part) {
      ...
      if (!el) {
        console.warn('math-exercise: feedback context "' + id + '" was not found.');
        return;
      }
      if (!el.classList.contains('math-feedback-context')) {
        console.warn(
          'math-exercise: element "' + id +
          '" is not a .math-feedback-context and was ignored.'
        );
        return;
      }
      ...
      if (!content) {
        console.warn('math-exercise: feedback context "' + id + '" is empty.');
        return;
      }
      if (usedChars + content.length > MAX_FEEDBACK_CONTEXT_CHARS) {
        console.warn(
          'math-exercise: feedback context "' + id +
  ...
  function buildUserPrompt(question, answer, contexts) {
    ...
    var contextParts = contexts.map(function (ctx) {
      return '<learning_context id="' + promptAttr(ctx.id) + '">\n' +
        ctx.content + '\n</learning_context>';
    });
  ...
  function setupCell(cell) {
    ...
    if (feedbackBtn) {
      feedbackBtn.addEventListener('click', function () {
        ...
        var contexts = collectFeedbackContexts(cell);
